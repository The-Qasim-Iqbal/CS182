...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out2, ar_cache = affine_relu_forward(out, W2, b2)
        scores, a_cache = affine_forward(out2, W3, b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss += 0.5 * self.reg * \
                np.sum([np.sum(np.square(self.params["W"+str(i)])) for i in range(1, 4)])


        dx3, gradW3, grads["b3"] = affine_backward(gradx, a_cache)
        dx2, gradW2, grads["b2"] = affine_relu_backward(dx3, ar_cache)
