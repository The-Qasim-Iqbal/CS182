...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Backward
        dh, dw_v, db_v = temporal_affine_backward(dx, ta_cache)
        if self.cell_type == 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            # 1
            # print(t)
            word_out, word_cache = word_embedding_forward(prev_word, W_embed)
            # print(word_out.shape)
            if 1 in word_out.shape:
                print('1 in shape, had to squeeze')
                word_out = np.squeeze(word_out, axis=1)
            # 2
            if self.cell_type == 'rnn':
                next_h, rnn_cache = rnn_step_forward(word_out, prev_h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                next_h, next_c, lstm_cache = lstm_step_forward(word_out, prev_h, prev_c, Wx, Wh, b)
                prev_c = next_c
            # 3
            ta_out, ta_cache = temporal_affine_forward(next_h[:, np.newaxis, :], W_vocab, b_vocab)
            # 4
            captions[:, t] = np.squeeze(np.argmax(ta_out, axis=2))
